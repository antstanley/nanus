...
| [`nanus-link`](../crates/nanus-link) | The local link: the frame vocabulary, the Unix-socket transport, the client an interface uses, and — behind a `server` feature — the half that serves an agent. |
